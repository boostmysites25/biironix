  ...
  return (
    <>
      {pathname.startsWith("/services/") && (
        <SEO
          title={allServices.find((s) => s.link === pathname)?.title || "Services"}
          description={allServices.find((s) => s.link === pathname)?.desc || "Explore our IT services."}
          keywords={`${allServices.find((s) => s.link === pathname)?.title}, IT Services, BIIRONIX TECH`}
        />
      )}
      <WebsiteHeader />
      ...
      <div ref={wrapperRef} className="wrapper">
        <div className="py-[5rem] grid md:grid-cols-[30%_65%] grid-cols-1 gap-10">
          <div className="w-full flex flex-col gap-10">
            <div
              data-aos="fade-up"
              className="bg-red-50 p-4 lg:p-6 rounded-lg"
            >
                ...
                {allServices.map((item) => (
                  <Link
                    ...
                    className={`${item.link === pathname
                      ...
                      : "bg-white text-black hover:bg-red-100 transition-colors"
            ...
            <Link to={`tel:${companyDetails.phone}`} className="font-semibold">
              <div
                data-aos="fade-up"
                className="text-center bg-gradient-to-b hidden md:flex flex-col items-center gap-3 text-white from-primary to-red-800 px-4 lg:px-6 py-10 rounded-lg shadow-lg"
              >
          ...
          <Link to={`tel:${companyDetails.phone}`} className="font-semibold">
            <div
              data-aos="fade-up"
              className="text-center bg-gradient-to-b flex md:hidden flex-col items-center gap-3 text-white from-primary to-red-800 px-4 lg:px-6 py-10 rounded-lg shadow-lg"
            >
              <div className="w-[3.5rem] h-[3.5rem] bg-white text-primary rounded-full p-3 flex justify-center items-center">
                <FaPhoneAlt className="text-3xl" />
              </div>
